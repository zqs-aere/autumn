...
function processRequest(request, response) {
  ...
  var pathName = path.join(url.parse(requestUrl).pathname);
  ...
  fs.stat(filePath, (err, stats) => {
    ...
    //没出错 并且文件存在
    if (!err && stats.isFile()) {
      readFile(filePath, contentType, response);
    }
    //如果路径是目录
      ...
      fs.readdir(filePath, (err, files) => {
        if (err) {
          ...
          for (var file of files) {
            ...
            if (file === "index.html") {
              readFile(filePath + (filePath[filePath.length - 1] == '/' ? '' : '/') + 'index.html', "text/html", response);
              flag = true;
              break;
            };
